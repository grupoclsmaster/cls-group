...
export default function EditEcosystemBannerPage() {
    ...
    const checkAdminAndLoad = async () => {
      try {
        ...
        if ((member && member.member_type === "admin") || isEmailAdmin) {
          ...
          if (!idStr) {
            showStatus("error", "ID do slide não fornecido.");
            return;
          }
          ...
          if (banner) {
            ...
            setTag(banner.tag || "");
            setCoverImageUrl(banner.image || "");
            setCtaText(banner.cta_text || "Acessar");
            setCtaLink(banner.cta_link || "#");
            setDisabled(banner.disabled || false);
          }
        } else {
          router.push("/sem-permissao");
        }
      } catch (err: any) {
        console.error("Erro ao carregar slide:", err);
        showStatus("error", err.message || "Erro ao carregar dados do slide.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSaveBanner = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showStatus("success", "Slide atualizado com sucesso!");
      setTimeout(() => {
  ...
  if (loading) {
    return (
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", minHeight: "80vh" }}>
        <p style={{ color: "var(--color-outline)" }}>Carregando dados do slide...</p>
      </div>
    );
  }
  ...
  return (
    <div className="animate-fadeIn" style={{ maxWidth: "1000px", margin: "0 auto", paddingBottom: "60px" }}>
      ...
      <section style={{ marginBottom: "24px", display: "flex", alignItems: "center", gap: "8px" }} className="font-label-caps">
        ...
        <span style={{ color: "var(--color-surface-variant)" }}>/</span>
        <span style={{ color: "var(--color-on-surface-variant)" }}>Editar Slide</span>
      </section>

      <h2 className="font-display-mobile" style={{ color: "var(--color-on-surface)", marginBottom: "8px" }}>Editar Slide</h2>
      <p className="font-body-lg" style={{ color: "var(--color-on-surface-variant)", marginBottom: "32px" }}>
        Atualize os textos, capa e links do slide da vitrine do ecossistema.
      </p>
