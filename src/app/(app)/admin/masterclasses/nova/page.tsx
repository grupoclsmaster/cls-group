...
export default function CreateMasterclassPage() {
  ...
  useEffect(() => {
    const checkAdmin = async () => {
      try {
        ...
        showStatus("error", err.message || "Erro ao validar permissões.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSaveMasterclass = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const sequence_order = (existingCourses?.length || 0) + 1;
      ...
      showStatus("success", `Masterclass "${title}" criada com sucesso!`);
      setTimeout(() => {
        router.push(`/admin/painel?courseId=${newCourse?.id || ""}`);
      }, 1000);
    } catch (err: any) {
  ...
  return (
    <div className="animate-fadeIn" style={{ maxWidth: "1000px", margin: "0 auto", paddingBottom: "60px" }}>
      ...
      <section style={{ marginBottom: "24px", display: "flex", alignItems: "center", gap: "8px" }} className="font-label-caps">
        ...
        <span style={{ color: "var(--color-on-surface-variant)" }}>Cadastrar Nova Masterclass</span>
      </section>

      <h2 className="font-display-mobile" style={{ color: "var(--color-on-surface)", marginBottom: "8px" }}>Cadastrar Nova Masterclass</h2>
      <p className="font-body-lg" style={{ color: "var(--color-on-surface-variant)", marginBottom: "32px" }}>
        Crie uma nova Masterclass definindo o seu nome inicial. Você poderá gerenciar seus módulos e aulas após criá-la.
      </p>
        ...
        <form onSubmit={handleSaveMasterclass} style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
            ...
            <input
              ...
              placeholder="Ex: Masterclass de Gestão e Negócios"
              value={title}
